...
function getdata(){
	...
	$.ajax({
            type: "get",
            async: false,
            url: "http://192.168.18.68:8000/policy/get_top_session/src_top_session/",
            dataType: "json",
            success: function(results){
                ...
                div = document.getElementById("src_session_top");
                div.innerHTML = "";
                for(var j=0; j<message.length; j++){
                    ...
                    child_div_name = "src_session_top_"+j.toString();
                    ...
                    draw_src_session_top(str_t,max_num,child_div_name,src_top_session_ydata,src_top_session_pointdata);
                }
...
function getdata_dst(){
    ...
    $.ajax({
        type: "get",
        async: false,
        url: "http://192.168.18.68:8000/policy/get_top_session/dst_top_session/",
        dataType: "json",
        success: function(results){
            message = results["message"];
            t = results["time"];
            ...
            var timestamp2 = Date.parse(new Date(t));
            timestamp2 = timestamp2 / 1000;
            div = document.getElementById("dst_session_top");
            div.innerHTML = "";
            max_session = 0;
            for(var j=0; j<message.length; j++){
                dst_top_session_ydata = [];
                ...
                var child_div = document.createElement("div");
                child_div_name = "dst_session_top_"+j.toString();
                child_div.setAttribute("id",child_div_name);
                child_div.setAttribute("style","height:300px");
                div.appendChild(child_div);
                var newDate = new Date();
                ...
                str_t = newDate.toLocaleString().replace(/:\d{1,2}$/,'');
                //alert(max_num);
                draw_dst_session_top(str_t,max_num,child_div_name,dst_top_session_ydata,dst_top_session_pointdata);
...
function draw_src_session_top(index,max_num,child_div_name,src_top_session_ydata,src_top_session_pointdata){
    //minute = index*2;
    //if(minute == 0)
        //title_str = "当前时间"
    //else
        //title_str = "前"+minute.toString()+"分钟时"
	var myChart = echarts.init(document.getElementById(child_div_name));
	var options={
                        title : {
                                text: '基于源的连接('+index+')',
                                subtext: '根据Session数排序的Top Ten',
                        },
                        tooltip : {
                            trigger: 'axis',
...
function draw_dst_session_top(index,max_session,child_div_name,src_top_session_ydata,src_top_session_pointdata){
    //minute = index*2;
    //if(minute == 0)
        //title_str = "当前时间";
    //else
